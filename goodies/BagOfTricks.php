 ...
 */
 ...
 function validateLoginForm($data){
   ...
   // This function validates an username regarding it's structure and content.
	function validateUsername($username, $minUsername, $maxUsername){
		$expression = '/^(?=[\W]+[a-zA-Z0-9]|[a-zA-Z0-9]+[\W]|[a-zA-Z0-9]+).{'. $minUsername . ',' . $maxUsername .'}$/';
	...
	function validatePassword($password, $minPassword, $maxPassword){
		$expression = '/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{' . $minPassword . ',' . $maxPassword . '}$/';
	...
	// This function validates an username regarding it's structure and content.
	function validatename($name, $minname, $maxname){
		$expression = '/^(?=[\W]+[a-zA-Z0-9]|[a-zA-Z0-9]+[\W]|[a-zA-Z0-9]+).{'. $minname . ',' . $maxname .'}$/';
		/* This expression in adapted from the web: it allows special and alfanumeric chars but only 
		 * in the presence of one another. Please note that \W means any word char (including special chars)
		 */ 
	...
	// This function validates an username regarding it's structure and content.
	function validatedescription($description, $mindesc, $maxdesc){
		$expression = '/^(?=[\W]+[a-zA-Z0-9]|[a-zA-Z0-9]+[\W]|[a-zA-Z0-9]+).{'. $mindesc . ',' . $maxdesc .'}$/';
		/* This expression in adapted from the web: it allows special and alfanumeric chars but only 
		 * in the presence of one another. Please note that \W means any word char (including special chars)
		 */ 
	...
	// This function validates an username regarding it's structure and content.
	function validateprice($price, $minprice, $maxprice){
		$expression = '/^([0-9]+).{'. $minprice . ',' . $maxprice .'}$/';
		/* This expression in adapted from the web: it allows special and alfanumeric chars but only 
		 * in the presence of one another. Please note that \W means any word char (including special chars)
		 */ 
	...
	// This function validates an username regarding it's structure and content.
	function validateimage($image, $minimage, $maximage) {
    $accepted_types = array('image/jpg', 'image/jpeg', 'image/png');
    return isset($image['type']) && in_array($image['type'], $accepted_types);
	}
