		...
		if ( ! is_array($validationResult) && ! is_string($validationResult) ){
         ...
         if ( is_string( $myDb) ){
         	...
         	// prepare and execute the MySQl statement to search for the username/password pair in the 'users' table.
				...
				$query='SELECT users.id_users, users.username, users.password, users_type_users.type_user
						FROM users
						LEFT JOIN users_type_users ON users.id_users = users_type_users.id_users
         		...
         		echo "This operation is unavailable right now. Please try again later (Code: 20)";
				   die(); 
					...
					elseif( mysqli_num_rows($result) == 0 ){
						// the combination between the username & password simply does not exist on the database table. The user should be informed.
